---
layout: page
title: Talks & Seminars
description: >
  Selected talks, seminars, and recorded presentations.
permalink: /talks/
sitemap: true
---

A selection of talks and seminars I have given. Recorded videos are
embedded where available.

## NeuroAI Seminar — KAIST AMILab

*The Virtuous Cycle of NeuroAI: Bridging Biological and Artificial Intelligence.*
**Feb 2026.**

{% include youtube.html id="pNfBXntZh10" title="NeuroAI Seminar — Virtuous Cycle of NeuroAI" quality="maxresdefault" %}

The talk frames NeuroAI as a two-direction loop — using AI to study
brains and behavior, and drawing on neuroscience to inform AI
architectures — and walks through three concrete projects (rodent
empathy, emotion-aware LLMs, conditional visual similarity in VLMs)
where this loop is operative.

## Other Talks

- **2025 (Mar)** Co-presenter, *Advancing Emotion Recognition in LLMs*, Impact Scholars Program Seminar (Neuromatch Academy), virtual.
- **2025 (Jun)** Workshop instructor, *Neuro-AI Convergence: Foundation Model Workflows for Neuroscience*, DBDL @ Dept. of BCS, KAIST.
- **2024 (Jan)** Invited speaker, *Behavioral Video Analysis for Predicting Attraction towards Others*, BC & Behavior Workshop, DBDL @ KAIST.
- **2023 (Aug)** Invited speaker, *Mice Hierarchy and Empathy Study*, 50th European Brain & Behaviour Society Meeting (EBBS), Amsterdam.
- **2022 (Jun)** Poster, *AVATAR: AI Vision Analysis for Three-Dimensional Action in Real-Time*, CV4Animals Workshop @ CVPR, New Orleans.
- **2019 (May)** Poster, *Intrinsic and Social Factors of Empathic Response*, Francis Crick Symposium, Cold Spring Harbor Lab (Suzhou).
- **2018 (Oct)** Oral, *NAS-Optimized CNN for Driver Monitoring with XAI*, ETRI AI System Development Seminar, Pangyo. **[Video below ↓](#ddsa-2018)**
- **2013 (Nov)** Poster, *The Precision of Contextual Familiarity Memory Is Attenuated in Remote Retrieval*, SfN Annual Meeting, San Diego.

## DDSA Driver Monitoring — Facial Emotion Recognition Demo (2018) {#ddsa-2018}

Award-winning system from the **DDSA driver safety competition** (2018):
a NAS-optimized CNN for real-time facial emotion recognition applied to
driver state monitoring, with Grad-CAM explainability for model
interpretability. **Grand Prize (DDSA), Encouragement Prize (ETRI).**

**Video title**: *데이터 사이언스 프로젝트 6 — Prediction and Feedback System Based on Facial Emotion Recognition in Video*

{% include youtube.html id="SJyrKreEzos" title="Prediction and Feedback System — Facial Emotion Recognition (2018)" quality="maxresdefault" %}
